...
BUILD_ID = "v33-basketball-nfl-local-data"
...
def load_player_log_database(league,player):
    ...
    dedupe=[c for c in ["GAME_ID","game_id","DATE","TEAM","OPP"] if c in out.columns]
    if dedupe: out=out.drop_duplicates(subset=dedupe,keep="first")
    if "DATE" in out.columns:
        out["_date_sort"]=pd.to_datetime(out["DATE"],errors="coerce",utc=True)
        out=out.sort_values("_date_sort",ascending=False).drop(columns="_date_sort",errors="ignore")
    return out

def merge_live_and_saved_player_logs(live,saved):
    if live is None or live.empty:
        return saved.copy() if saved is not None else pd.DataFrame()
    if saved is None or saved.empty:
        return live.copy()
    a=live.copy(); b=saved.copy()
    for frame in [a,b]:
        if "DATE" not in frame.columns and "GAME_DATE" in frame.columns:
            frame["DATE"]=frame["GAME_DATE"]
        if "OPP" not in frame.columns and "MATCHUP" in frame.columns:
            frame["OPP"]=frame["MATCHUP"].map(_opponent_from_matchup_text)
    out=pd.concat([a,b],ignore_index=True,sort=False)
    keys=[c for c in ["EVENT_ID","DATE","MATCHUP","OPP"] if c in out.columns]
    if keys:
...
def local_nfl_player_log_v33(player,team=""):
    ...
    try:
        ...
        if isinstance(payload,dict):
            ...
            if isinstance(direct,list):
                ...
                if not df.empty:
                    return clean_injury_rows(df),None
            found=[]
...
def render_last20_prop_chart(gamelog,prop_spec,side,line,opponent="",title="Last 20 games"):
    if gamelog is None or gamelog.empty:
        st.caption("No game log available for the chart.")
        return
    vals=extract_values(gamelog,prop_spec)
    n=min(20,len(gamelog))
    idxs=gamelog.index[:n]
    if len(idxs)==0:
        st.caption("No game log available for the chart.")
        return
    dates=(gamelog.loc[idxs,"DATE"].astype(str) if "DATE" in gamelog.columns else pd.Series(range(n),index=idxs).astype(str))
    chart=pd.DataFrame({
        "Game":dates.values,
        "Stat":pd.to_numeric(vals.loc[idxs],errors="coerce").values,
        "Line":float(line),
    }).dropna(subset=["Stat"])
    if chart.empty:
        st.caption("The selected stat was not available in the recent game log.")
        return
    chart=chart.iloc[::-1].copy()
    st.markdown(f"### {title}")
    st.line_chart(chart.set_index("Game")[["Stat","Line"]],use_container_width=True)
    check=checkmark_prop_table(gamelog,prop_spec,side,line,opponent)
    c1,c2,c3=st.columns(3)
    c1.metric("Last 5",fmt_pct(hit_rate(vals.head(5),side,line)))
    c2.metric("Last 10",fmt_pct(hit_rate(vals.head(10),side,line)))
    c3.metric("Last 20",fmt_pct(hit_rate(vals.head(20),side,line)))
    with st.expander("Show the 20 game-by-game results"):
...
def _full_player_game_data(game,league,row):
    """Unified player loader used by full-game scans: live sources first, saved DB second."""
    name=str(row["player"])
    ...
    try:
        ...
        return {
            "name":name,
            "team":abbr,
            "opponent":result.get("opponent",opponent),
        ...
        return {
            ...
            "log":pd.DataFrame(),"matchup":pd.DataFrame(),"error":str(e),
        }
...
def refresh_player_history_database_v32(row,league,selected_date=None):
    ...
    if league in {"NBA","WNBA"}:
        ...
        local=local_nfl_player_log_v33(pname,pteam)
        if not local.empty:
    ...
    if not log.empty:
        ...
        added=upsert_player_log_database(log,league=league,player=pname,team=pteam,source="admin refresh")
...
def fetch_player_research_v23(row,game,league):
    ...
    if not log.empty:
        ...
        try:
            added=upsert_player_log_database(log,league=league,player=pname,team=pteam,source="automatic verified feed")
            if added:
                notes.append(f"Saved {added} new game row(s) to the local player database.")
        except Exception as e:
            notes.append(f"Local database save: {e}")

    # Saved rows are a reliability layer: live feeds refresh them, but a temporary
    # endpoint outage does not erase games the app has already collected/imported.
    try:
        saved=load_player_log_database(league,pname)
        if not saved.empty and log.empty:
            notes.append("Live player feed was unavailable; using the saved local game-log database.")
        log=merge_live_and_saved_player_logs(log,saved)
    except Exception as e:
        notes.append(f"Local database read: {e}")
...
st.caption(
    "v33 integrated build • separate Basketball/NFL workspaces • local seed data • recency-weighted models • sportsbook-confirmed Kalshi edge alerts."
)
    ...
    if not game:
        ...
        if not log.empty:
            ...
            if league=="NFL":
                ...
                recent_cols=[c for c in ["DATE","MATCHUP","OPP","MIN","PTS","REB","AST","FG3M","FG3A","TOV","RESULT","SEASON"] if c in log.columns]
                if recent_cols:
